...
abstract class MultiSelectFragment(private val matomoCategory: String) : Fragment(), MultiSelectResult {
    ...
    fun openMultiSelect() {
        multiSelectManager.multiSelectMode = true
    ...
    private fun sendActions(
        ...
        val canBulkAllSelectedFiles = multiSelectManager.allSelected && fileCount > BulkOperationsUtils.MIN_SELECTED
        ...
        if (areAllFromTheSameFolder && (canBulkAllSelectedFiles || hasEnoughSelectedFilesToBulk)) {
            sendBulkAction(
                fileCount, BulkOperation(
                    action = type,
                    fileIds = if (multiSelectManager.allSelected) null else selectedFiles.map { it.id },
