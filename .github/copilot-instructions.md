...
## Overview
kDrive for Android — cloud storage app. Fragment + Jetpack Compose hybrid UI, Hilt DI, Realm (offline file cache) + Room (auth tokens), Ktor networking. Three build flavors: `standard`, `fdroid`, `preprod`. GeniusScan SDK (document scanning, `standard` only), Paho MQTT (real-time updates). Tests use JUnit 5 via the `junit5` Gradle plugin.
...
# For instrumented tests only (CI auto-generates these):
touch app/src/androidTest/java/com/infomaniak/drive/utils/Env.kt
touch app/src/standard/java/com/infomaniak/drive/GeniusScanEnv.kt
```
...
- Realm is initialized with `runBlocking { initRealm() }` in `MainApplication` — any Realm access before this completes will crash.
